...
// @license      MIT
// @downloadURL  https://github.com/wuyaos/greasyfork_scripts/raw/refs/heads/main/dist/PT_AuditAssistant.user.js
// @updateURL    https://github.com/wuyaos/greasyfork_scripts/raw/refs/heads/main/dist/PT_AuditAssistant.user.js
...
(() => {
  ...
  // src/scripts/pt-audit-assistant/config.js
  var ID = "pt-audit-assistant";
  var AUTO_APPROVAL_KEY = "pt_audit_assistant_auto_approval";
  var AUTO_CLOSE_KEY = "pt_audit_assistant_auto_close";
  var DEBUG_KEY = "pt_audit_assistant_debug";
  var ONE_TIB = 1024 ** 4;
  var DEFAULT_COLLECTORS = ["title", "desc", "siteMeta", "tags", "mediainfo", "screenshots", "size", "approvalLink"];
  var DEFAULT_PARSERS = ["title", "mediainfo", "dbLinks", "doubanScore", "derived"];

  // src/scripts/pt-audit-assistant/page.js
  ...
  function toBytes(text) {
    ...
    const map = { b: 1, kb: 1e3, kib: 1024, mb: 1e3 ** 2, mib: 1024 ** 2, gb: 1e3 ** 3, gib: 1024 ** 3, tb: 1e3 ** 4, tib: 1024 ** 4, pb: 1e3 ** 5, pib: 1024 ** 5 };
  ...
  __name(normalizeResolution, "normalizeResolution");
  function isTagSet(ctx, tag) {
    return Boolean(ctx.tags.normalized && ctx.tags.normalized.has(tag));
  }
  __name(isTagSet, "isTagSet");
  function getTorrentId() {
    const p = new URLSearchParams(location.search);
    return p.get("id") || p.get("torrent_id") || (location.href.match(/[?&]id=(\d+)/) || [])[1] || "";
  }
  __name(getTorrentId, "getTorrentId");
  ...
  // src/scripts/pt-audit-assistant/registries.js
  ...
  var SiteRegistry = createRegistry("SiteRegistry");
  var CollectorRegistry = createRegistry("CollectorRegistry");
  var ParserRegistry = createRegistry("ParserRegistry");
  var ApprovalRegistry = createRegistry("ApprovalRegistry");
  var RuleRegistry = createRegistry("RuleRegistry");
  function createContext() {
    return {
      ...
      tags: { raw: "", normalized: /* @__PURE__ */ new Set(), flags: {} },
  ...
  // src/scripts/pt-audit-assistant/approval.js
  var ApprovalGate = {
  ...
  // src/scripts/pt-audit-assistant/renderer.js
  var Renderer = {
    init() {
      addStyle(`
...
#${ID}-panel.ptaa-error{background:#ffebee;color:#8a1111;border-color:#f44336}#${ID}-panel.ptaa-warning{background:#fff8db;color:#5f4300;border-color:#ffca28}#${ID}-panel .ptaa-title{font-weight:700;margin-bottom:6px}#${ID}-panel ul{margin:6px 0 6px 20px;padding:0}#${ID}-panel li{margin:2px 0}#${ID}-panel .ptaa-disclaimer{font-size:12px;opacity:.78;margin-top:6px}.ptaa-debug{white-space:pre-wrap;max-height:280px;overflow:auto;background:rgba(0,0,0,.06);padding:8px;border-radius:4px;margin-top:8px}`);
  ...
  function snapshotContext(ctx) {
    const copy = JSON.parse(JSON.stringify(ctx, (key, value) => key === "actionContainer" || key === "nativeReviewLink" || key === "images" ? void 0 : value));
    copy.tags.normalized = Array.from(ctx.tags.normalized || []);
    return copy;
  }
  __name(snapshotContext, "snapshotContext");

  // src/scripts/pt-audit-assistant/sites.js
  function override(rules, id, patch) {
    return rules.map((r) => r.id === id ? Object.assign({}, r, patch) : r);
  }
  __name(override, "override");
  function baseRules() {
    return [
      { id: "mediainfoRequired" },
      { id: "screenshotMinCount", params: { min: 1 } },
      { id: "screenshotValid", params: { severity: "warning", minHeight: 24, timeout: 3e4 } },
      { id: "dbLinkRequired", severity: "warning", params: { severity: "warning" } },
      { id: "officialLabelConsistency" },
      { id: "hdrTagsMatchMediainfo" },
      { id: "titleMediaMatches" },
      { id: "completeTagRequired" },
      { id: "incompleteTagRequired" },
      { id: "collectionTagForbidden" },
      { id: "audioLanguageTagConsistency" },
      { id: "vcbStudioTagRequired" },
      { id: "remuxTagRequired" },
      { id: "mediainfoBbcodeError" },
      { id: "mediainfoTypeConsistency" },
      { id: "musicTitleFields" }
    ];
  }
  __name(baseRules, "baseRules");
  function registerAllSites() {
    ...
    SiteRegistry.register("longpt", Object.assign({}, common, { id: "longpt", name: "LongPT", hosts: ["longpt.org"], reviewInfoPosition: 2, tagTextMap: { "高码": "highBitrate", "高碼": "highBitrate", "高帧": "highFps", "高幀": "highFps", "高分": "highScore" }, rules: [{ id: "longptDomDegraded" }, { id: "screenshotValid", params: { severity: "warning", minHeight: 24, timeout: 3e4 } }, { id: "highBitrate", params: { tag: "highBitrate", thresholdsMbps: { 2160: 15, 1080: 9, 1440: 9 }, inclusive: false } }, { id: "highFps", params: { thresholdFps: 60, tag: "highFps" } }, { id: "highScore", params: { threshold: 8, inclusive: false, tag: "highScore" } }] }));
  }
  __name(registerAllSites, "registerAllSites");

  // src/scripts/pt-audit-assistant/media-parsing.js
  function videoSection(raw) {
    const lines = String(raw || "").split(/\r?\n/);
    let start = -1;
    for (let i = 0; i < lines.length; i++) {
      if (/^\s*(Video|视频|視訊)\s*(?:#\d+)?\s*$/i.test(lines[i]) || /^\s*(Video|视频|視訊)\s*[:：]/i.test(lines[i])) {
        start = i;
        break;
      }
    }
  ...
  __name(videoSection, "videoSection");
  function parseBitrate(text) {
    ...
    if (u === "kb/s" || u === "kbit/s" || u === "kbps") return n / 1e3;
    if (u === "mb/s" || u === "mbit/s" || u === "mbps") return n;
    if (u === "gb/s" || u === "gbps") return n * 1e3;
  ...
  __name(languageMatches, "languageMatches");
  function escapeRegExp(s) {
    return String(s).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  }
  __name(escapeRegExp, "escapeRegExp");

  // src/scripts/pt-audit-assistant/rules.js
  ...
  __name(defaultTagTextMap, "defaultTagTextMap");
  function registerRules() {
    RuleRegistry.register("requiredSubtitle", { run: /* @__PURE__ */ __name((ctx) => ctx.siteMeta.subtitle ? [] : finding("error", "SUBTITLE_REQUIRED", "副标题必填"), "run") });
    RuleRegistry.register("requiredMedium", { run: /* @__PURE__ */ __name((ctx) => ctx.parsed.medium || ctx.debug.metaSelections?.medium ? [] : finding("error", "MEDIUM_REQUIRED", "媒介信息缺失或无法解析"), "run") });
    RuleRegistry.register("requiredVideoCodec", { run: /* @__PURE__ */ __name((ctx) => ctx.parsed.codec || ctx.debug.metaSelections?.codec ? [] : finding("error", "VIDEO_CODEC_REQUIRED", "视频编码缺失或无法解析"), "run") });
    RuleRegistry.register("requiredResolution", { run: /* @__PURE__ */ __name((ctx) => ctx.parsed.resolution || ctx.debug.metaSelections?.resolution ? [] : finding("error", "RESOLUTION_REQUIRED", "分辨率缺失或无法解析"), "run") });
    ...
    RuleRegistry.register("screenshotMinCount", { run: /* @__PURE__ */ __name((ctx, site, params) => ctx.screenshots.count >= (params.min || 1) ? [] : finding(params.severity || "error", "SCREENSHOT_MIN_COUNT", `截图数量不足，要求至少 ${params.min || 1} 张，当前 ${ctx.screenshots.count} 张`), "run") });
    ...
    RuleRegistry.register("mediainfoRequired", { run: /* @__PURE__ */ __name((ctx) => !ctx.mediainfo.empty ? [] : finding("error", "MEDIAINFO_REQUIRED", "MediaInfo 必填或采集失败"), "run") });
    RuleRegistry.register("dbLinkRequired", { run: /* @__PURE__ */ __name((ctx, site, params) => ctx.dbLinks.hasAny ? [] : finding(params.severity || "error", "DB_LINK_REQUIRED", "简介中未检测到 IMDb / 豆瓣 / TMDB / Bangumi 链接或信息"), "run") });
    ...
    RuleRegistry.register("highFps", { run: /* @__PURE__ */ __name((ctx, site, params) => ctx.parsed.fps != null && ctx.parsed.fps >= (params.thresholdFps || 60) && !isTagSet(ctx, params.tag || "highFps") ? finding("error", "HIGH_FPS_TAG_MISSING", `帧率 ${ctx.parsed.fps} fps 达到高帧阈值，未选择高帧标签`) : [], "run") });
    RuleRegistry.register("highScore", { run: /* @__PURE__ */ __name((ctx, site, params) => ctx.doubanScore != null && (params.inclusive ? ctx.doubanScore >= (params.threshold || 8) : ctx.doubanScore > (params.threshold || 8)) && !isTagSet(ctx, params.tag || "highScore") ? finding("error", "HIGH_SCORE_TAG_MISSING", `豆瓣评分 ${ctx.doubanScore} 超过高分阈值，未选择高分标签`) : [], "run") });
    RuleRegistry.register("titleChineseForbidden", { run: /* @__PURE__ */ __name((ctx) => ctx.title.hasChinese ? finding("error", "TITLE_CHINESE_FORBIDDEN", "标题含中文字符") : [], "run") });
    RuleRegistry.register("titleChineseWarning", { run: /* @__PURE__ */ __name((ctx) => ctx.title.hasChinese ? finding("warning", "TITLE_CHINESE_WARNING", "标题含中文字符，请确认是否符合站点规范") : [], "run") });
    RuleRegistry.register("titleCompleteRequiredForSeason", { run: /* @__PURE__ */ __name((ctx) => ctx.title.isSeasonPack && !ctx.title.hasComplete ? finding("error", "TITLE_COMPLETE_REQUIRED", "季包标题应包含 Complete") : [], "run") });
    RuleRegistry.register("titleHdr10PlusSpelling", { run: /* @__PURE__ */ __name((ctx) => /HDR10Plus/i.test(ctx.title.clean) ? finding("error", "HDR10PLUS_SPELLING", "HDR10+ 建议使用 HDR10+ 写法，不使用 HDR10Plus") : [], "run") });
    RuleRegistry.register("forbidGroups", { run: /* @__PURE__ */ __name((ctx, site, params) => (params.groups || []).some((g) => new RegExp(`[-@]${escapeRegExp(g)}$`, "i").test(ctx.title.clean)) ? finding("error", "FORBID_GROUP", "标题命中禁发组，请人工核对") : [], "run") });
    RuleRegistry.register("bigTorrentWarning", { run: /* @__PURE__ */ __name((ctx) => ctx.size.isBiggerThan1T && !isTagSet(ctx, "bigTorrent") ? finding("warning", "BIG_TORRENT_TAG_MISSING", "种子大于等于 1 TiB，建议确认大包标签") : [], "run") });
    ...
    RuleRegistry.register("completeTagRequired", { run: /* @__PURE__ */ __name((ctx) => ctx.title.isSeasonPack && ctx.title.hasComplete && !isTagSet(ctx, "complete") ? finding("warning", "COMPLETE_TAG_MISSING", "季包已标注 Complete，建议选择完结标签") : [], "run") });
    RuleRegistry.register("incompleteTagRequired", { run: /* @__PURE__ */ __name((ctx) => ctx.title.isEpisode && !isTagSet(ctx, "incomplete") && !isTagSet(ctx, "complete") ? finding("warning", "INCOMPLETE_TAG_MISSING", "分集剧集未选择分集标签") : [], "run") });
    RuleRegistry.register("collectionTagForbidden", { run: /* @__PURE__ */ __name((ctx) => !ctx.title.isSeasonPack && isTagSet(ctx, "collection") ? finding("warning", "COLLECTION_TAG_EXTRA", "非季包资源选择了合集标签，请确认") : [], "run") });
    ...
    RuleRegistry.register("vcbStudioTagRequired", { run: /* @__PURE__ */ __name((ctx) => ctx.derived.vcbStudioSeed && !isTagSet(ctx, "vcbStudio") ? finding("warning", "VCB_STUDIO_TAG_MISSING", "VCB-Studio 资源未选择 VCB-Studio 标签") : [], "run") });
    RuleRegistry.register("remuxTagRequired", { run: /* @__PURE__ */ __name((ctx) => /remux/i.test(ctx.parsed.medium) && !isTagSet(ctx, "remux") ? finding("warning", "REMUX_TAG_MISSING", "Remux 媒介未选择 Remux 标签") : [], "run") });
    ...
    RuleRegistry.register("requiredAudioCodec", { run: /* @__PURE__ */ __name((ctx) => ctx.parsed.audio || ctx.debug.metaSelections?.audio ? [] : finding("error", "AUDIO_CODEC_REQUIRED", "音频编码缺失或无法解析"), "run") });
    RuleRegistry.register("mediainfoBbcodeError", { run: /* @__PURE__ */ __name((ctx) => ctx.mediainfo.containsBBCode ? finding("error", "MEDIAINFO_BBCODE", "MediaInfo 含 BBCode 标签，应为纯文本") : [], "run") });
    ...
    RuleRegistry.register("diyUntouchedTagConsistency", {
      run(ctx) {
        if (ctx.mediainfo.kind !== "bdinfo") return [];
    ...
    RuleRegistry.register("hlgNeedsHdrTag", { run: /* @__PURE__ */ __name((ctx) => /^(?!Encoding).*HLG/im.test(ctx.mediainfo.raw) && !isTagSet(ctx, "hdr") && !isTagSet(ctx, "hdrVivid") ? finding("warning", "HLG_HDR_TAG_MISSING", "MediaInfo 检测到 HLG，未选择 HDR 标签") : [], "run") });
    RuleRegistry.register("titleAudioChannelRequired", { run: /* @__PURE__ */ __name((ctx) => ctx.title.audioToken && !ctx.title.hasAudioChannel ? finding("warning", "TITLE_AUDIO_CHANNEL_MISSING", "标题音频编码后未标示声道数（如 5.1）") : [], "run") });
    ...
    RuleRegistry.register("dvdResolutionCheck", { run: /* @__PURE__ */ __name((ctx) => /\bDVD\b/i.test(ctx.title.clean) && ctx.parsed.resolutionHeight === 720 ? finding("warning", "DVD_RESOLUTION_CHECK", "DVD 来源资源标为 720p，请检查分辨率是否错标") : [], "run") });
    RuleRegistry.register("longptDomDegraded", { run: /* @__PURE__ */ __name((ctx) => !ctx.title.clean || !$("#outer") && !$("#kdescr") ? finding("info", "DOM_DEGRADED", "页面 DOM 与默认 NexusPHP 选择器不完全匹配，部分检查已降级") : [], "run") });
  }
  __name(registerRules, "registerRules");

  // src/scripts/pt-audit-assistant/collectors.js
  function registerCollectors() {
      ...
      run(ctx, site) {
        ...
        if (tagCell) {
          ...
          if (leaves.length) tagTexts = Array.from(leaves).map((n) => clean(n.textContent)).filter(Boolean);
            ...
            const t = clean(tagCell.textContent);
            if (t) tagTexts = [t];
          }
        }
        ...
        const norm = /* @__PURE__ */ __name((s) => String(s || "").replace(/\s+/g, "").toLowerCase(), "norm");
    ...
    CollectorRegistry.register("screenshots", {
      async run(ctx, site) {
        ...
        const timeout = (site.rules || []).find((r) => r.id === "screenshotValid")?.params?.timeout || 3e4;
        const states = await Promise.all(images.map((img) => new Promise((resolve) => {
          ...
          const done = /* @__PURE__ */ __name((broken) => resolve({ url, naturalWidth: img.naturalWidth || 0, naturalHeight: img.naturalHeight || 0, complete: Boolean(img.complete), broken: Boolean(broken) }), "done");
          if (img.complete) {
  ...
  __name(registerCollectors, "registerCollectors");

  // src/scripts/pt-audit-assistant/parsers.js
...
${urls.join("\n")}`;
  ...
  __name(registerParsers, "registerParsers");

  // src/scripts/pt-audit-assistant/approval-registration.js
  function registerApprovalRegistration() {
    ApprovalRegistry.register("nexusPhpTokenPost", {
      async run(ctx, site) {
        ...
        const page = await gmRequest({ method: "GET", url: pageUrl, timeout: 15e3 });
        ...
        const res = await gmRequest({ method: "POST", url: submitUrl, data: body, headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8", "Accept": "application/json, text/plain, */*" }, timeout: 15e3 });
  ...
  __name(registerApprovalRegistration, "registerApprovalRegistration");

  // src/scripts/pt-audit-assistant/startup.js
  function registerMenu() {
    if (typeof GM_registerMenuCommand !== "function") return;
    GM_registerMenuCommand(`${getValue(AUTO_APPROVAL_KEY, false) ? "关闭" : "开启"}自动审批（默认关）`, () => {
      setValue(AUTO_APPROVAL_KEY, !getValue(AUTO_APPROVAL_KEY, false));
      location.reload();
    });
    GM_registerMenuCommand(`${getValue(AUTO_CLOSE_KEY, false) ? "关闭" : "开启"}自动关闭页面`, () => {
      setValue(AUTO_CLOSE_KEY, !getValue(AUTO_CLOSE_KEY, false));
      location.reload();
    });
    GM_registerMenuCommand(`${getValue(DEBUG_KEY, false) ? "关闭" : "开启"}显示调试信息(ctx 快照)`, () => {
      setValue(DEBUG_KEY, !getValue(DEBUG_KEY, false));
      location.reload();
    });
  }
  __name(registerMenu, "registerMenu");
  async function boot() {
    registerAllSites();
    registerMenu();
  ...
  __name(boot, "boot");
  function bootstrap() {
    SiteRegistry.detect = /* @__PURE__ */ __name(function detect() {
    ...
    CollectorRegistry.run = /* @__PURE__ */ __name(async function runCollectors(ids, ctx, site) {
      const results = [];
    ...
    RuleRegistry.run = /* @__PURE__ */ __name(function runRules(bindings, ctx, site) {
      const findings = [];
      ...
      (site.suppressions || []).forEach((s) => {
        ...
        if (ids.length && categoryId && ids.includes(categoryId) || texts.length && categoryText && texts.some((v) => lower(categoryText).includes(lower(v)))) {
          (bindings || []).forEach((b) => {
            if (!(s.exceptRules || []).includes(b.id)) suppressed.add(b.id);
          });
        }
      });
  ...
  // src/scripts/pt-audit-assistant/index.js
  bootstrap();
})();
