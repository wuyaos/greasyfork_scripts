...
(() => {
  ...
  // src/scripts/zhuque-batch-download/config.js
  var ID = "zhuque-batch-dl";
  var STORAGE_KEY = "zhuque_batch_dl_cfg";
  var DELAY_KEY = "zhuque_batch_dl_delay";
  var UNIT_BYTES = { kib: 1024, mib: 1024 ** 2, gib: 1024 ** 3, tib: 1024 ** 4, pib: 1024 ** 5 };
  var state = { rows: [], filtered: [], panel: null, ui: {}, mountTimer: null, lastSig: "" };

  // src/scripts/zhuque-batch-download/elements.js
  ...
  function append(parent, ...kids) {
    kids.forEach((k) => parent.append(k));
    return parent;
  }
  ...
  __name(clean, "clean");
  function pageIsDark() {
    const color = firstPaintedBg(document.querySelector(".ant-layout-content") || document.body);
    return color ? colorBrightness(color) < 128 : false;
  }
  __name(pageIsDark, "pageIsDark");
  function firstPaintedBg(node) {
    for (let el2 = node; el2 && el2 !== document.documentElement; el2 = el2.parentElement) {
      const bg = getComputedStyle(el2).backgroundColor;
      if (bg && !/^rgba?\(\s*0\s*,\s*0\s*,\s*0\s*,\s*0\s*\)$/i.test(bg) && bg !== "transparent") return bg;
    }
  ...
  function colorBrightness(color) {
    ...
    if (!m) return 255;
    return (Number(m[1]) * 299 + Number(m[2]) * 587 + Number(m[3]) * 114) / 1e3;
  }
  __name(colorBrightness, "colorBrightness");
  function parseMult(primary, fallback, arrow) {
    const re = new RegExp(`${arrow}\\s*([\\d.]+)x`);
    const raw = String(primary || "").match(re)?.[1] ?? String(fallback || "").match(re)?.[1];
  ...
  function defaultSizeUnit(value) {
    return clean(value).match(/([KMGTP]iB)$/i)?.[1] || "MiB";
  }
  ...
  function sanitize(value) {
    const s = clean(value).replace(/[\\/:*?"<>|]+/g, "_").slice(0, 120);
    return s || "torrent";
  }
  ...
  // src/scripts/zhuque-batch-download/settings.js
  function getRows() {
    return [...document.querySelectorAll(".ant-table-row")].map(readRow).filter(Boolean);
  }
  ...
  function saveCfg(c) {
    try {
      GM_setValue(STORAGE_KEY, c);
    } catch (_) {
    }
  }
  __name(saveCfg, "saveCfg");
  ...
  function registerMenu() {
    try {
      GM_registerMenuCommand("设置下载延迟(ms)", () => {
        const cur = safeGet(DELAY_KEY, 800);
        const v = prompt("每个种子下载间隔毫秒数（最小 100）：", cur);
        if (v != null && /^\d+$/.test(String(v).trim())) GM_setValue(DELAY_KEY, Math.max(100, parseInt(v, 10)));
      });
    } catch (_) {
    }
  }
  ...
  // src/scripts/zhuque-batch-download/download.js
  ...
  __name(downloadOne, "downloadOne");
  function batchDownload(items, btn) {
    ...
    const next = /* @__PURE__ */ __name(() => {
      if (i >= items.length) {
        ...
        return setStatus(`完成：成功 ${ok} / 失败 ${fail} / 共 ${items.length}`);
      }
      const item = items[i++];
      setStatus(`下载中 ${i}/${items.length}`);
      downloadOne(item, (success) => {
        if (success) ok++;
        else fail++;
  ...
  // src/scripts/zhuque-batch-download/controls.js
  function multiFilter(title, selected) {
    const root = el("details");
    const summary = el("summary");
    const box = el("div", { class: "zq-menu" });
  ...
  function fillMulti(control, values, selected) {
    ...
    [...counts.keys()].sort((a, b) => parseFloat(a) - parseFloat(b) || a.localeCompare(b)).forEach((value) => {
      const cb = el("input", { type: "checkbox", value });
      cb.checked = old.has(value);
      control.box.append(el("label", {}, cb, ` ${value} (${counts.get(value)})`));
    });
    updateMultiSummary(control);
  }
  __name(fillMulti, "fillMulti");
  function selectedMulti(control) {
    return control ? [...control.box.querySelectorAll("input:checked")].map((x) => x.value) : [];
  }
  __name(selectedMulti, "selectedMulti");
  function updateMultiSummary(control) {
    const count = selectedMulti(control).length;
    control.summary.textContent = count ? `已选 ${count}` : "全部";
  }
  __name(updateMultiSummary, "updateMultiSummary");
  function sizeRangeField(label, min, minUnit, max, maxUnit) {
    return el("div", { class: "zq-field zq-range-field zq-size-field" }, el("label", {}, label), el("div", { class: "zq-range" }, el("div", { class: "zq-range-box zq-size-box" }, min, minUnit), el("span", { class: "zq-range-sep" }, "~"), el("div", { class: "zq-range-box zq-size-box" }, max, maxUnit)));
  }
  __name(sizeRangeField, "sizeRangeField");
  function rangeField(label, min, max) {
    return el("div", { class: "zq-field zq-range-field" }, el("label", {}, label), el("div", { class: "zq-range" }, el("div", { class: "zq-range-box zq-num-box" }, min), el("span", { class: "zq-range-sep" }, "~"), el("div", { class: "zq-range-box zq-num-box" }, max)));
  }
  __name(rangeField, "rangeField");
  function selectField(label, control) {
    return el("div", { class: "zq-field zq-select-field" }, el("label", {}, label), control);
  }
  __name(selectField, "selectField");
  function input(type, placeholder, className, value, title) {
    return el("input", { type, class: className, placeholder, value: normalizeInputValue(value), title });
  }
  __name(input, "input");
  function unitSelect(value) {
    const sel = el("select", { class: "zq-unit" });
    ["KiB", "MiB", "GiB", "TiB", "PiB"].forEach((u) => {
  ...
  function button(label, fn, cls = "") {
    const b = el("button", { class: `zq-btn ${cls}`.trim(), type: "button" }, label);
    b.addEventListener("click", fn);
    return b;
  }
  __name(button, "button");

  // src/scripts/zhuque-batch-download/selection.js
  function actionsRow() {
  ...
  __name(selectFiltered, "selectFiltered");
  function invertChecks() {
    document.querySelectorAll(".zq-row-check input[data-id]").forEach((cb) => {
      cb.checked = !cb.checked;
    });
    setStatus(`已勾选 ${getChecked().length} 条`);
  }
  __name(invertChecks, "invertChecks");
  function getChecked() {
  ...
  __name(injectRowChecks, "injectRowChecks");
  function syncChecks(items) {
    const ids = new Set(items.map((r) => r.id));
    document.querySelectorAll(".zq-row-check input[data-id]").forEach((cb) => {
      cb.checked = ids.has(cb.getAttribute("data-id"));
    });
  }
  __name(syncChecks, "syncChecks");
  function highlight(items) {
    const ids = new Set(items.map((r) => r.id));
    state.rows.forEach((r) => {
      r.row.style.background = ids.has(r.id) ? "rgba(48,126,255,.10)" : "";
    });
  }
  __name(highlight, "highlight");

  // src/scripts/zhuque-batch-download/styles.js
...
@media(max-width:760px){#${ID} .zq-filter-line{grid-template-columns:1fr}#${ID} #zq-batch-status{margin-left:0;text-align:left;flex-basis:100%}}
  ...
  // src/scripts/zhuque-batch-download/panel.js
  function buildPanel() {
    ensureStyle();
    state.rows = getRows();
    state.filtered = [];
    ...
    state.ui.seedMin = input("number", "最小", "zq-size", cfg.seedMin);
    state.ui.seedMax = input("number", "最大", "zq-size", cfg.seedMax);
    state.ui.leechMin = input("number", "最小", "zq-size", cfg.leechMin);
    state.ui.leechMax = input("number", "最大", "zq-size", cfg.leechMax);
    ...
    head.lastChild.addEventListener("click", () => {
      ...
      head.lastChild.textContent = open ? "展开 ▼" : "收起 ▲";
    });
    state.panel = box;
    refreshOptions(cfg);
    return box;
  ...
  // src/scripts/zhuque-batch-download/mount.js
  function mount() {
    if (document.querySelector("#" + ID)) {
      refreshRowsAndOptions();
      return;
    }
  ...
  // src/scripts/zhuque-batch-download/startup.js
  function bootstrap() {
    start();
  }
  __name(bootstrap, "bootstrap");

  // src/scripts/zhuque-batch-download/index.js
  bootstrap();
})();
