...
// Generated from src/scripts/moviepilot-name-test/index.js; do not edit dist files.
"use strict";
(() => {
  var __defProp = Object.defineProperty;
  var __name = (target, value) => __defProp(target, "name", { value, configurable: true });

  // src/scripts/moviepilot-name-test/config.js
  var SCRIPT_NAME = GM_info && GM_info.script ? GM_info.script.name : "Moviepilot Script";
  var MTEAM_API_BASE = "https://api.m-team.cc/api";
  var MTEAM_DETAIL_TTL = 5 * 60 * 1e3;
  var mteamDetailCache = /* @__PURE__ */ new Map();
  var mteamDetailPending = /* @__PURE__ */ new Map();
  var CONSTANTS = {
    API_ENDPOINTS: {
  ...
  // src/scripts/moviepilot-name-test/media-formatting.js
  var UTILS = {
    parseSize(sizeStr) {
    ...
    normalizeTorrentTitle(rawTitle) {
      ...
      title = title.replace(/\.torrent$/i, "").replace(/\.(mkv|mp4|avi|ts|m2ts|flv|wmv)$/i, "");
      for (let i = 0; i < 6; i++) {
        const next = title.replace(/^\s*(?:\[[^\]]{1,30}\]|\([^\)]{1,30}\)|【[^】]{1,30}】|<[^>]{1,30}>)(?:[\s._-]+|$)/, "");
        if (next === title) break;
        title = next;
      }
      title = title.replace(/^[^A-Za-z0-9\u4e00-\u9fa5]+/, "").replace(/^[A-Za-z0-9]{2,10}\]\s*/, "").replace(/^[\]\)\】\}]+/, "");
    ...
    stripEpisodeInfo(title) {
      return String(title || "").replace(/\bS\d{1,2}E\d{1,3}\b/ig, " ").replace(/\bE\d{1,3}\b/ig, " ").replace(/\b第\s*\d+\s*[季集]\b/g, " ").replace(/\s+/g, " ").trim();
    },
    extractSubtitleCandidates(subtitle) {
      ...
      const clean = String(subtitle || "").replace(/\*[^*]{1,40}\*/g, " ").replace(/\[[^\]]{1,40}\]/g, " ").replace(/[|｜]/g, "/").replace(/\s+/g, " ").trim();
      clean.split(/\s*\/\s*/).forEach((part) => {
        let p = String(part || "").trim();
        if (!p) return;
        p = p.replace(/(?:评论音轨|多国语字幕|字幕|中字|簡繁|简繁|国语|日语|英语|粤语|双语|音轨|內封|外挂|內嵌).*/gi, "").replace(/\s+/g, " ").trim();
        add(p);
    ...
    getRecognitionCandidates(rawTitle, subtitle = "") {
      const candidates = [];
      const addCandidate = /* @__PURE__ */ __name((title) => {
        const val = String(title || "").replace(/\s+/g, " ").replace(/\.torrent$/i, "").replace(/\.(mkv|mp4|avi|ts|m2ts|flv|wmv)$/i, "").replace(/^[A-Za-z0-9]{2,10}\]\s*/, "").trim();
      ...
      const hasTvPattern = /\bS\d{1,2}E\d{1,3}\b/i.test(normalized) || /\bE\d{1,3}\b/i.test(normalized) || /第\s*\d+\s*[季集]/.test(normalized);
      ...
      if (!hasTvPattern) {
        ...
        if (yearMatch) {
          ...
          const noYear = normalized.replace(new RegExp(`\\b${year}\\b`, "g"), " ").replace(/\(\s*\)/g, " ").replace(/\s+/g, " ").trim();
          addCandidate(noYear);
          if (noYear) {
  ...
  var UI = {
    ...
    _addModalEventListeners(isInitialSetup) {
      ...
      const toggleAuthFields = /* @__PURE__ */ __name(() => {
      ...
      const cancelAction = /* @__PURE__ */ __name(() => {
        if (isInitialSetup) {
        ...
        try {
          ...
          if (authMode === "apikey") {
            ...
            const loginRes = await new Promise((resolve, reject) => GM_xmlhttpRequest({
              method: "POST",
              url: `${mpUrl}/api/v1/login/access-token`,
    ...
    _getConfigModalHTML() {
      ...
      const secretInput = /* @__PURE__ */ __name((id, value = "", placeholder = "") => this._secretInputHTML(id, value, placeholder), "secretInput");
      return `
                <h2>MoviePilot 配置</h2>
  ...
  // src/scripts/moviepilot-name-test/settings.js
  var CONFIG = {
    _values: {},
    load() {
...
这将清除所有存储的 Moviepilot 设置并刷新页面。`)) {
        ...
        try {
          GM_deleteValue(CONSTANTS.RECOGNIZE_CACHE.KEY);
        } catch (e) {
        }
...
${root?.textContent || ""}`.match(/Hash[：:]\s*([a-fA-F0-9]{40})/)?.[1] || "";
    ...
    __name(needsDownloadForHash, "needsDownloadForHash");
    ...
    return { DOM, Mount: Mount2, SITE_FAMILIES: SITE_FAMILIES2, TORRENT_LINK_SELECTORS, needsDownloadForHash, tableMount: tableMount2, AutoFeedAnchors: AutoFeedAnchors2, AdapterRuntime: AdapterRuntime2, GazelleSites: GazelleSites2, GazellePicker: GazellePicker2 };
  }, "createPTCommon");

  // src/scripts/moviepilot-name-test/page-tools.js
  var { DOM: PTDOM, Mount, SITE_FAMILIES, tableMount, AutoFeedAnchors, AdapterRuntime, GazelleSites, GazellePicker } = createPTCommon({
  ...
  // src/scripts/moviepilot-name-test/api.js
  var API = {
    _sessionToken: null,
      ...
      try {
        ...
        const tokenResponse = await new Promise((resolve, reject) => {
          GM_xmlhttpRequest({
            ...
            onerror: /* @__PURE__ */ __name((err) => reject(new Error("生成下载令牌时发生网络错误")), "onerror")
          });
        });
  ...
  // src/scripts/moviepilot-name-test/bt-pages.js
  var BT_SITE_HELPERS = {
  ...
  // src/scripts/moviepilot-name-test/cache.js
  ...
  __name(normalizeDownloadKey, "normalizeDownloadKey");
  var Cache = {
    _data: null,
    _load() {
    ...
    _prune() {
      ...
      if (keys.length > max) {
        keys.map((k) => ({ k, ts: data[k]?.ts || 0 })).sort((a, b) => a.ts - b.ts).slice(0, keys.length - max).forEach(({ k }) => delete data[k]);
      }
    },
    clear() {
      this._data = {};
  ...
  // src/scripts/moviepilot-name-test/core.js
  var Core = {
    async handlePage() {
      try {
      ...
      try {
        ...
        if (torrentInfo.extra?.groupMode) {
          ...
          for (const entry of entries) {
            ...
            const cached2 = Cache.get(pickedInfo);
            if (!cached2?.media_info) continue;
            GM_log(`[${SCRIPT_NAME}] 命中识别缓存: ${pickedInfo.name}`);
            this.renderSuccess(container, cached2, pickedInfo);
            return;
          }
          return;
        }
    ...
    _processOneTorrent(torrentInfo) {
      try {
        ...
        if (container.parentElement?.querySelectorAll?.(".mp-recognize-trigger").length > 1) {
          container.remove();
          return;
        }
        const torrentData = {
    ...
    renderGroupPicker(container, torrentInfo) {
      ...
      const panel = GazellePicker.buildPanel({
        ...
        rows: entries,
        actionLabel: "选择并识别",
        onAction: /* @__PURE__ */ __name(async (entry, btn) => {
          btn.disabled = true;
          btn.textContent = "读取中…";
    ...
    renderManualEntry(container, torrentInfo, state = "idle", message = "待识别") {
      ...
      const reselect = torrentInfo.extra?.groupMode === false && (torrentInfo.extra?.entries || []).length > 1 ? `<button type="button" class="mp-reselect-torrent" style="background-color:${CONSTANTS.COLORS.SECONDARY}; color:white; border:none; border-radius:4px; font:inherit; line-height:1.45; font-weight:600; cursor:pointer; padding:.12em .6em;">重选种子</button>` : "";
    ...
    startOrPick(container, torrentInfo) {
      ...
      const liveTorrentInfo = liveEntries.length > 1 ? { ...torrentInfo, extra: { ...torrentInfo.extra || {}, groupMode: true, entries: liveEntries, groupTitle: torrentInfo.extra?.groupTitle || torrentInfo.name, currentTid: torrentInfo.extra?.tid || "" } } : torrentInfo;
    ...
    async startRecognition(container, torrentInfo) {
      const setRunning = /* @__PURE__ */ __name((msg) => {
        const trigger = container.querySelector(".mp-recognize-trigger");
        if (trigger) {
    ...
    async tmdbFallback(candidates, subtitle) {
      try {
        const uniqueQueries = [];
        const addQuery = /* @__PURE__ */ __name((q) => {
    ...
    async handleDownload(button, media_info, torrentInfo) {
      ...
      const log = /* @__PURE__ */ __name((msg, ...args) => GM_log(`[${SCRIPT_NAME}] [Download] ${msg}`, ...args), "log");
      const setStatus = /* @__PURE__ */ __name((text) => {
        button.textContent = text;
        UI.showToast(text, 2e3);
        log(text);
      }, "setStatus");
      try {
        if (torrentInfo._bangumiId && !torrentInfo.downloadLink) {
          ...
          try {
            const apiRes = await new Promise((resolve, reject) => GM_xmlhttpRequest({
              ...
              responseType: "json",
              onload: /* @__PURE__ */ __name((r) => r.status === 200 ? resolve(r.response) : reject(new Error(`API ${r.status}`)), "onload"),
              onerror: reject
            }));
            if (apiRes?.magnet) {
              let trackerParams = "";
  ...
  // src/scripts/moviepilot-name-test/adapters.js
  var SITE_ADAPTERS = [
    {
      id: "totheglory",
      matches: /* @__PURE__ */ __name(() => window.location.href.includes("totheglory.im/t/"), "matches"),
      ...AdapterRuntime.withMount(
        ...
        (mount) => {
          ...
          const sizeString = firstOf(rows, (row) => row.textContent.includes("尺寸"))?.nextElementSibling?.innerText || "";
    ...
    {
      id: "hdsky",
      matches: /* @__PURE__ */ __name(() => window.location.href.includes("hdsky.me/details.php"), "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "sjtu",
      matches: /* @__PURE__ */ __name(() => window.location.href.includes("pt.sjtu.edu.cn/details.php"), "matches"),
      ...AdapterRuntime.withMount(
        () => {
          const rows = document.querySelectorAll(".rowhead, .heading");
          if (rows.length < 4) return null;
    ...
    {
      id: "hdcity",
      matches: /* @__PURE__ */ __name(() => window.location.href.includes("hdcity.city/t-"), "matches"),
      ...AdapterRuntime.withMount(
    ...
    {
      id: "monikadesign",
      matches: /* @__PURE__ */ __name(() => window.location.hostname === "monikadesign.uk" && /^\/torrents\/\d+\/?$/.test(window.location.pathname), "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "bangumi-moe",
      matches: /* @__PURE__ */ __name(() => window.location.hostname === "bangumi.moe", "matches"),
      ...AdapterRuntime.withMount(
        () => {
        ...
        (mount) => {
          ...
          const root = modal || document;
          const titleEl = root.querySelector("a.title-link b") || root.querySelector('a[href*="/torrent/"]');
          const title = titleEl?.textContent?.trim() || BT_SITE_HELPERS.text(".torrent-title, .subject-title, .title") || document.title.replace(/\s*[-|_].*$/, "").trim();
          const downloadLink = root.querySelector('a[href^="magnet:"]')?.href || BT_SITE_HELPERS.findDownloadLink(['a[href^="magnet:"]', 'a[href*="/download/"]', 'a[href*=".torrent"]']);
    ...
    {
      id: "mikanani",
      matches: /* @__PURE__ */ __name(() => window.location.hostname === "mikanani.me" && window.location.pathname.startsWith("/Home/"), "matches"),
      ...AdapterRuntime.withMount(
        () => {
        ...
        (mount) => {
          const rawTitle = BT_SITE_HELPERS.text(".episode-title, h1, h2, .an-text") || document.title.replace(/\s*-\s*Mikan Project\s*$/, "").trim();
    ...
    {
      id: "comicat-kisssub",
      matches: /* @__PURE__ */ __name(() => /(^|\.)(comicat|kisssub)\.org$/i.test(window.location.hostname), "matches"),
      ...AdapterRuntime.withMount(
        () => {
        ...
        (mount) => {
          ...
          const encodedMagnet = Array.from(document.querySelectorAll('a[href*="magnet%3A"], a[href*="magnet%3a"]')).map((el) => el.href.match(/magnet%3A.*$/i)?.[0]).filter(Boolean)[0];
          const downloadLink = BT_SITE_HELPERS.findDownloadLink([
            'a[href^="magnet:"]',
            'a[href*=".torrent"]'
    ...
    {
      id: "acg-rip",
      matches: /* @__PURE__ */ __name(() => window.location.hostname === "acg.rip", "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "nyaa",
      matches: /* @__PURE__ */ __name(() => window.location.hostname === "nyaa.si", "matches"),
      ...AdapterRuntime.withMount(
        () => /^\/view\/\d+$/.test(window.location.pathname) ? Mount.afterNode(document.querySelector(".panel-heading") || document.body) : null,
        (mount) => {
          const title = BT_SITE_HELPERS.text("h3.panel-title") || document.title.replace(/\s*::\s*Nyaa\s*$/i, "").trim();
          const downloadLink = BT_SITE_HELPERS.findDownloadLink([
            'a[href^="magnet:"]',
            'a[href*="/download/"]',
            'a[href*=".torrent"]'
    ...
    {
      id: "beyond-hd",
      matches: /* @__PURE__ */ __name(() => window.location.hostname === "beyond-hd.me" && window.location.pathname.startsWith("/torrents/"), "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "eiga",
      matches: /* @__PURE__ */ __name(() => window.location.hostname === "eiga.moi" && window.location.pathname.startsWith("/torrents/"), "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "hd-space",
      matches: /* @__PURE__ */ __name(() => window.location.hostname === "hd-space.org" && window.location.search.includes("page=torrent-details"), "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "iptorrents",
      matches: /* @__PURE__ */ __name(() => window.location.hostname === "iptorrents.com" && window.location.pathname === "/torrent.php", "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "filelist",
      matches: /* @__PURE__ */ __name(() => window.location.hostname === "filelist.io" && window.location.pathname === "/details.php", "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "hudbt",
      matches: /* @__PURE__ */ __name(() => window.location.hostname === "hudbt.hust.edu.cn" && window.location.pathname === "/details.php", "matches"),
      ...AdapterRuntime.withMount(
        () => {
          const dl = document.querySelector('a.index[href*="download.php?id="]') || document.querySelector('a[href*="download.php?id="]');
    ...
    {
      id: "hhclub",
      matches: /* @__PURE__ */ __name(() => window.location.hostname === "hhanclub.net" && window.location.pathname === "/details.php", "matches"),
      ...AdapterRuntime.withMount(
        () => {
          const dl = document.querySelector('a.index[href*="download.php?id="]') || document.querySelector('a[href*="download.php?id="]');
          const row = dl?.closest("tr");
    ...
    {
      id: "generic-nexusphp",
      matches: /* @__PURE__ */ __name(() => document.querySelector(".rowhead") && !window.location.href.includes("totheglory.im") && !window.location.href.includes("hdsky.me") && !window.location.href.includes("hdcity.city"), "matches"),
      ...AdapterRuntime.withMount(
        () => {
          const rows = document.querySelectorAll(".rowhead");
    ...
    {
      id: "m-team",
      matches: /* @__PURE__ */ __name(() => /m-team\.(cc|io|vip)\/detail\//.test(window.location.href), "matches"),
  ...
  // src/scripts/moviepilot-name-test/site.js
  var Site = {
    adapter: null,
    init() {
      this.adapter = null;
  ...
  // src/scripts/moviepilot-name-test/routing.js
  var isMTeamHost = /* @__PURE__ */ __name(() => /(^|\.)m-team\.(cc|io|vip)$/.test(window.location.hostname), "isMTeamHost");
  var isMTeamDetail = /* @__PURE__ */ __name(() => isMTeamHost() && window.location.pathname.startsWith("/detail/"), "isMTeamDetail");

  // src/scripts/moviepilot-name-test/startup.js
  function main() {
    CONFIG.load();
    ...
    let hasBooted = false;
    const bootCore = /* @__PURE__ */ __name(async (options = {}) => {
    ...
    const installMTeamDynamicBoot = /* @__PURE__ */ __name(() => {
      let bootedTorrentId = "";
      let booting = false;
      let bootTimer = null;
      const currentTorrentId = /* @__PURE__ */ __name(() => window.location.pathname.match(/\/detail\/(\d+)/)?.[1] || "", "currentTorrentId");
      const hasMTeamAnchor = /* @__PURE__ */ __name(() => Array.from(document.querySelectorAll("label")).some((label) => label.textContent.trim() === "字幕"), "hasMTeamAnchor");
      const requestBoot = /* @__PURE__ */ __name((delay = 300) => {
        clearTimeout(bootTimer);
        bootTimer = setTimeout(tryBoot, delay);
      }, "requestBoot");
      let attempts = 0;
      const tryBoot = /* @__PURE__ */ __name(async () => {
    ...
    if (Site.adapter.id === "m-team") {
      ...
      const spaBoot = /* @__PURE__ */ __name(() => {
        let attempts = 0;
        const tryBoot = /* @__PURE__ */ __name(() => {
      ...
      const onUrlChange = /* @__PURE__ */ __name(() => {
  ...
  // src/scripts/moviepilot-name-test/index.js
  bootstrap();
})();
