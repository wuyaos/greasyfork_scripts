...
// Generated from src/scripts/iyuu-reseed-checker/index.js; do not edit dist files.
"use strict";
(() => {
  var __defProp = Object.defineProperty;
  var __name = (target, value) => __defProp(target, "name", { value, configurable: true });

  // src/scripts/iyuu-reseed-checker/config.js
  var SCRIPT_NAME = "IYUU 辅种检测助手";
  var API_BASE = "https://2025.iyuu.cn";
  var ZMPT_API = "https://zmpt.cc/nodeapi/iyuu/getIyuuByInfoHash";
  var MTEAM_API_BASE = "https://api.m-team.cc/api";
  var KEYS = {
    token: "iyuu_reseed_token",
    owned: "iyuu_reseed_owned_sites",
    zmpt: "iyuu_reseed_zmpt_enabled",
    sites: "iyuu_reseed_sites_index",
    sitesTime: "iyuu_reseed_sites_index_time",
    sid: "iyuu_reseed_sid_sha1",
    sidTime: "iyuu_reseed_sid_sha1_time",
    sidKey: "iyuu_reseed_sid_sha1_key",
    result: "iyuu_reseed_result_cache",
    mteamKey: "iyuu_reseed_mteam_api_key",
    configured: "iyuu_reseed_configured_once",
    autoQuery: "iyuu_reseed_auto_query",
    gazelleDl: "iyuu_reseed_gazelle_download_enabled",
    mpUrl: "moviepilotUrl",
    mpUser: "moviepilotUser",
    mpPass: "moviepilotPassword",
    mpAuthMode: "moviepilotAuthMode",
    mpApiKey: "moviepilotApiKey"
  };
  var COLORS = { primary: "#2775b6", secondary: "#e6702e", success: "#5bb053", warn: "#c54640", info: "#677489" };
  var safeJson = /* @__PURE__ */ __name((v) => {
    try {
      return JSON.stringify(v, (k, val) => val instanceof ArrayBuffer ? `ArrayBuffer(${val.byteLength})` : val, 2);
    } catch (_) {
      return String(v);
    }
  }, "safeJson");
  var log = /* @__PURE__ */ __name((label, value = "") => {
    GM_log(`[${SCRIPT_NAME}] ${label}`, value && typeof value === "object" ? safeJson(value) : value);
  }, "log");
  var wait = /* @__PURE__ */ __name((ms) => new Promise((resolve) => setTimeout(resolve, ms)), "wait");
  var normalizeSiteKey = /* @__PURE__ */ __name((value) => String(value || "").toLowerCase().replace(/[\s._-]+/g, ""), "normalizeSiteKey");

  // src/scripts/iyuu-reseed-checker/settings.js
  var Store = {
    get(k, d = "") {
      try {
...
确定重置所有配置和缓存吗？`)) return;
  ...
  // src/scripts/iyuu-reseed-checker/site-catalog.js
  var AUTO_FEED_SITE_DEFS = [
  ...
  var normalizedHost = /* @__PURE__ */ __name((raw) => {
    try {
  ...
  var AUTO_FEED_SITE_URLS = Object.fromEntries(AUTO_FEED_SITE_DEFS.flatMap((def) => (def.aliases || []).flatMap((alias) => [
    [String(alias || "").toLowerCase(), def.url],
    [normalizeSiteKey(alias), def.url]
  ]).filter(([key]) => key)));
  var AUTO_FEED_HOST_URLS = Object.fromEntries(AUTO_FEED_SITE_DEFS.flatMap((def) => [def.url, ...def.hosts || []].map((raw) => [normalizedHost(raw), def.url]).filter(([host]) => host)));
  var AUTO_FEED_CANONICAL_HOSTS = Object.fromEntries(AUTO_FEED_SITE_DEFS.flatMap((def) => {
  ...
  // src/common/first-match.js
  var firstOf = /* @__PURE__ */ __name((items, predicate) => {
  ...
  // src/scripts/iyuu-reseed-checker/site-index.js
  var SiteIndex = {
    async get(force, tokenOverride = "") {
      const ts = Number(Store.get(KEYS.sitesTime, 0));
      const cached = Store.json(KEYS.sites, []);
      if (!force && cached.length && Date.now() - ts < 7 * 864e5) return this.applyOverrides(cached);
      const token = tokenOverride || Config.token;
      if (!token) return this.applyOverrides(cached);
      let last = null;
      for (let i = 0; i < 3; i++) {
        const res = await HTTP.request({ url: `${API_BASE}/reseed/sites/index`, headers: { Token: token, Accept: "application/json" } });
        log("sites index response", res);
        last = this.parse(res);
        if (last.length) {
          Store.set(KEYS.sites, JSON.stringify(last));
          Store.set(KEYS.sitesTime, Date.now());
          return this.applyOverrides(last);
        }
        await wait(1e3 + i * 700);
      }
    ...
    name(s) {
      return s?.nickname || s?.name_cn || s?.name || s?.site || s?.domain || `站点${s?.id || s?.sid || ""}`;
    },
    aliases(s) {
    ...
    searchText(s) {
      const fields = [
        ...
        s?.url,
        this.host(s),
        this.host({ url: this.autoFeedUrl(s) }),
        ...this.aliases(s)
      ];
    ...
    icon(s) {
      ...
      const addCandidate = /* @__PURE__ */ __name((raw) => {
        if (!raw) return false;
        try {
          const text = String(raw);
    ...
    homepage(s) {
      const raw = s?.base_url || s?.url || s?.domain || s?.site || "";
      if (!raw) return "";
      try {
        return (raw.startsWith("http") ? new URL(raw) : new URL(`https://${raw}`)).origin;
      } catch (_) {
        const host = this.host(s);
    ...
    webOrigin(s) {
      const origin = this.homepage(s);
      return /(^|\.)m-team\.(cc|io|vip)$/i.test(this.host(s)) ? "https://kp.m-team.cc" : origin;
    },
    ...
    webUrl(url) {
      try {
        ...
        if (/(^|\.)m-team\.(cc|io|vip)$/i.test(u.hostname.replace(/^api\./, ""))) {
          u.protocol = "https:";
          u.host = "kp.m-team.cc";
        }
        return u.href;
    ...
    pageUrl(s, pageField, torrentId, rawUrl = "", web = false) {
      const origin = web ? this.webOrigin(s) : this.homepage(s);
      const direct = rawUrl || "";
      if (!origin) return web ? this.webUrl(direct) : direct;
      if (!torrentId) return web ? this.webUrl(direct || origin) : direct || origin;
      const tpl = pageField || "details.php?id={}";
      const path = String(tpl).replace("{}", String(torrentId)).replace(/\{[^}]*\}/g, "");
      const built = path.startsWith("http") ? path : `${origin}/${path.replace(/^\//, "")}`;
      const out = direct && !this.isHomepageUrl(direct) ? direct : built;
      return web ? this.webUrl(out) : out;
    },
    detailUrl(s, torrentId, rawUrl = "") {
      return this.pageUrl(s, s?.details_page || "details.php?id={}", torrentId, rawUrl, true);
    },
    downloadUrl(s, torrentId, rawUrl = "") {
      const direct = rawUrl || "";
      if (direct && !this.isHomepageUrl(direct)) return direct;
      if (this.isTotheglory(s)) return "";
      return torrentId ? this.pageUrl(s, s?.download_page || "download.php?id={}", torrentId, "") : "";
    },
    isLikelyDetailUrl(url) {
      return /(?:details\.php|\/detail\/|torrents\.php\?id=)/i.test(String(url || ""));
    },
    ...
    isMTeam(s) {
      const text = [s?.host, s?.url, s?.downloadUrl, s?.domain, s?.base_url, s?.site, s?.name, s?.nickname, s?.name_cn].filter(Boolean).join(" ");
      return /m-team\.(cc|io|vip)|馒头|饅頭|\bM[-_ ]?Team\b|\bMT\b/i.test(text);
    },
    isTotheglory(s) {
      const text = [s?.host, s?.url, s?.downloadUrl, s?.domain, s?.base_url, s?.site, s?.name, s?.nickname, s?.name_cn].filter(Boolean).join(" ");
      return /totheglory\.im|(^|\b)TTG\b/i.test(text);
    },
    mTeamApi() {
      return `${MTEAM_API_BASE}/torrent/genDlToken`;
    },
    canonicalHost(host) {
    ...
    hasCurrent(list) {
      return Boolean(this.currentSid(list));
    },
    bySid(list) {
      const m = /* @__PURE__ */ new Map();
      (list || []).forEach((s) => m.set(String(s.id || s.sid), s));
      return m;
    },
    matchMoviePilot(mpSites, iyuuSites) {
      const hosts = new Set((mpSites || []).map((s) => this.host(s)).filter(Boolean));
      return (iyuuSites || []).filter((s) => {
        const host = this.host(s);
        return host && [...hosts].some((h) => this.sameHost(h, host));
      }).map((s) => String(s.id || s.sid)).filter(Boolean);
    },
    async detectLoggedIn(list, onProgress) {
      const targets = (list || []).filter((s) => this.host(s) && (s.id || s.sid));
      const hits = [];
      let done = 0;
      const concurrency = 3;
      const gap = 400;
      const probe = /* @__PURE__ */ __name(async (s) => {
        const sid = String(s.id || s.sid);
        const host = this.host(s);
        try {
          const html = await HTTP.request({ url: `https://${host}/`, responseType: "text", allowStatuses: [301, 302, 403] });
          if (!/login|登录|sign\s*in|忘记密码|忘記密碼|验证码|captcha|two[-_ ]?step/i.test(String(html || ""))) hits.push(sid);
        } catch (_) {
        }
        done++;
  ...
  // src/scripts/iyuu-reseed-checker/http.js
  var iyuuLastRequest = 0;
  var HTTP = {
    allowed(raw) {
      try {
        const u = new URL(raw, location.origin);
        const h = u.hostname;
        if (h === "2025.iyuu.cn" || h === "zmpt.cc" || h === "api.m-team.cc" || h === location.hostname || /(^|\.)m-team\.(cc|io|vip)$/.test(h) || h === "bangumi.moe" || h === "totheglory.im") return true;
        const mp = MoviePilot.cfg().url;
        if (mp && h === new URL(mp).hostname.toLowerCase()) return true;
        return SiteIndex.applyOverrides(Store.json(KEYS.sites, [])).some((s) => {
          const host = SiteIndex.host(s);
          return host && (h === host || h.endsWith(`.${host}`) || host.endsWith(`.${h}`));
        });
      } catch (_) {
        return false;
      }
    ...
    request({ method = "GET", url, data, headers = {}, responseType = "json", allowStatuses = [] }) {
      ...
      const run = /* @__PURE__ */ __name(async () => {
        if (new URL(full).hostname === "2025.iyuu.cn") {
          const gap = Date.now() - iyuuLastRequest;
          if (gap < 800) await wait(800 - gap);
          iyuuLastRequest = Date.now();
        }
        log("HTTP request", { method, url: full, responseType, allowStatuses });
        return new Promise((resolve, reject) => GM_xmlhttpRequest({ method, url: full, data, headers, responseType, timeout: 15e3, onload: /* @__PURE__ */ __name((r) => {
          log("HTTP response", { url: full, status: r.status, response: r.response });
          return r.status >= 200 && r.status < 300 || allowStatuses.includes(r.status) ? resolve(r.response) : reject(new Error(`HTTP ${r.status}`));
        }, "onload"), onerror: reject, ontimeout: /* @__PURE__ */ __name(() => reject(new Error("请求超时")), "ontimeout") }));
      }, "run");
      return run();
    },
  ...
  // src/scripts/iyuu-reseed-checker/moviepilot.js
  var MoviePilot = {
    cfg() {
      return { url: String(Store.get(KEYS.mpUrl, "") || "").replace(/\/$/, ""), user: Store.get(KEYS.mpUser, ""), pass: Store.get(KEYS.mpPass, ""), auth: Store.get(KEYS.mpAuthMode, "password"), key: Store.get(KEYS.mpApiKey, "") };
    },
    save(modal) {
      Store.set(KEYS.mpUrl, modal.querySelector("#iyuuMpUrl").value.trim().replace(/\/$/, ""));
      Store.set(KEYS.mpAuthMode, modal.querySelector("#iyuuMpAuth").value);
      Store.set(KEYS.mpUser, modal.querySelector("#iyuuMpUser").value.trim());
      Store.set(KEYS.mpPass, modal.querySelector("#iyuuMpPass").value);
      Store.set(KEYS.mpApiKey, modal.querySelector("#iyuuMpApiKey").value.trim());
    },
    async headers() {
      const c = this.cfg();
      if (!c.url) throw new Error("请先配置 MoviePilot 地址");
      if (c.auth === "apikey") return { "X-API-KEY": c.key, Accept: "application/json" };
      const res = await HTTP.request({ method: "POST", url: `${c.url}/api/v1/login/access-token`, data: `username=${encodeURIComponent(c.user)}&password=${encodeURIComponent(c.pass)}`, headers: { "Content-Type": "application/x-www-form-urlencoded" } });
      if (!res?.access_token) throw new Error("MoviePilot 登录失败");
      return { Authorization: `bearer ${res.access_token}`, Accept: "application/json" };
    },
    async sites() {
      const c = this.cfg();
      const headers = await this.headers();
      const res = await HTTP.request({ url: `${c.url}/api/v1/site`, headers });
      log("MoviePilot sites response", res);
      const raw = res?.data || res?.items || res?.list || res;
      return Array.isArray(raw) ? raw : Object.values(raw || {});
    }
  };

  // src/scripts/iyuu-reseed-checker/cache.js
  var ResultCache = {
    _data: null,
    version: "site-dedupe-v2",
    okTtl: 6 * 36e5,
    partialTtl: 36e5,
    max: 500,
    _load() {
      ...
      try {
        const raw = Store.get(KEYS.result, "{}");
    ...
    _persist() {
      try {
        Store.set(KEYS.result, JSON.stringify(this._data || {}));
      } catch (e) {
        log("result cache write failed", e.message);
      }
    },
    sidKey() {
      return Config.owned.map(Number).filter(Boolean).sort((a, b) => a - b).join(",");
    },
    key(hash) {
      return `hash:${hash}|${this.sidKey()}`;
    },
    normalizeUrl(url) {
      try {
        const u = new URL(url || location.href, location.origin);
        u.hash = "";
        ["authkey", "torrent_pass", "usetoken", "https", "passkey"].forEach((key) => u.searchParams.delete(key));
        u.searchParams.sort();
        return u.href;
      } catch (_) {
        return String(url || "");
    ...
    downloadUrlKey(url) {
      try {
        ...
        if (id && (/download/i.test(u.pathname) || /download/i.test(action))) return `${u.origin}${u.pathname}?action=${action || "download"}&id=${id}`;
      } catch (_) {
      }
      return "";
    },
    ...
    pageKey(input = "") {
      ...
      return `page:${this.detailUrlKey(url) || this.downloadUrlKey(url) || this.normalizeUrl(url)}|${this.sidKey()}`;
    },
    siteKeys(payload) {
      return (payload?.sites || []).flatMap((s) => [s?.url, s?.downloadUrl].filter(Boolean).map((url) => this.pageKey(url)));
    },
    _ttlOf(item) {
      return item?.partial ? this.partialTtl : this.okTtl;
    },
    getKey(key) {
      const data = this._load();
      const item = data[key];
      if (!item || item.version !== this.version) return null;
      const ttl = this._ttlOf(item);
      if (Date.now() - (item.ts || 0) > ttl) {
        delete data[key];
        this._persist();
        return null;
      }
    ...
    get(hash) {
      return this.getKey(this.key(hash));
    },
    getPage(url = "") {
      return this.getKey(this.pageKey(url));
    },
    set(hash, payload, info) {
      if (!hash || !payload) return;
      const data = this._load();
      const item = { payload, ts: Date.now(), version: this.version, partial: Boolean(payload.partial) };
      data[this.key(hash)] = item;
      data[this.pageKey()] = item;
      this.siteKeys(payload).forEach((k) => {
        data[k] = item;
      });
      this._prune();
      this._persist();
    ...
    _prune() {
      ...
      Object.keys(data).forEach((k) => {
        const item = data[k] || {};
        const ttl = this._ttlOf(item);
        if (now - (item.ts || 0) > ttl) delete data[k];
      });
      const keys = Object.keys(data);
      if (keys.length > this.max) keys.map((k) => ({ k, ts: data[k]?.ts || 0 })).sort((a, b) => a.ts - b.ts).slice(0, keys.length - this.max).forEach(({ k }) => delete data[k]);
    },
    clear() {
      this._data = {};
      Store.del(KEYS.result);
      UI.toast("辅种查询缓存已清空");
    },
    size() {
  ...
  // src/scripts/iyuu-reseed-checker/ui.js
  var UI = {
    initStyle() {
      GM_addStyle(`
    ...
    actionText(action, status) {
      return `${action}（${status}）`;
    },
    tag(text, color = COLORS.info) {
      const el = document.createElement("span");
      el.className = "iyuu-chip source";
    ...
    siteChip(site, selected, multi = false) {
      const wrap = document.createElement(multi ? "label" : site.url ? "a" : "span");
      wrap.className = "iyuu-chip iyuu-site-choice";
      if (multi && selected) {
        const cb = document.createElement("input");
        cb.type = "checkbox";
        cb.checked = selected.has(site);
        cb.onchange = () => cb.checked ? selected.add(site) : selected.delete(site);
        wrap.appendChild(cb);
      } else if (site.url) {
        wrap.href = site.url;
        wrap.target = "_blank";
      ...
      inner.className = "iyuu-site-link";
      inner.append(this.icon(site.name, site.icon), document.createTextNode(site.name));
    ...
    avatar(name) {
      ...
      s.className = "iyuu-avatar";
      s.textContent = String(name || "?").trim().slice(0, 1).toUpperCase();
      return s;
    },
    toast(msg) {
      const t = document.createElement("div");
      t.className = "iyuu-toast";
      t.textContent = msg;
      document.body.appendChild(t);
      setTimeout(() => t.remove(), 2600);
    },
    eyeIcon(hidden = true) {
      return hidden ? '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.8 12s3.2-5.5 9.2-5.5S21.2 12 21.2 12s-3.2 5.5-9.2 5.5S2.8 12 2.8 12Z"/><circle cx="12" cy="12" r="2.4"/></svg>' : '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.8 12s3.2-5.5 9.2-5.5c1.7 0 3.2.45 4.45 1.1M21.2 12s-3.2 5.5-9.2 5.5c-1.65 0-3.1-.4-4.3-1.05M4.5 4.5l15 15"/><path d="M9.9 9.9a2.4 2.4 0 0 0 3.2 3.2"/></svg>';
    },
    ...
    async safeRun(label, fn) {
      try {
        ...
        this.toast(`${label}失败：${e?.message || "未知错误"}`);
      }
    },
    async showConfig() {
      let sites = await SiteIndex.get(false).catch(() => []);
      const bg = document.createElement("div");
      bg.className = "iyuu-modal-bg";
      const modal = document.createElement("div");
      modal.className = "iyuu-modal";
      ...
      const updateMpAuthFields = /* @__PURE__ */ __name(() => {
      ...
      const renderSites = /* @__PURE__ */ __name((kw = "") => {
        const query = normalizeSiteKey(kw);
        grid.textContent = "";
        sites.filter((s) => !query || SiteIndex.searchText(s).includes(query)).forEach((s) => {
          const sid = String(s.id || s.sid);
          if (!sid) return;
          const b = document.createElement("button");
          b.type = "button";
          b.className = `iyuu-site-chip${selected.has(sid) ? " selected" : ""}`;
          b.dataset.sid = sid;
          b.title = SiteIndex.searchTitle(s);
          b.append(this.icon(SiteIndex.name(s), SiteIndex.icon(s)), document.createTextNode(SiteIndex.name(s)));
          b.onclick = () => {
      ...
      renderSites();
      modal.querySelector("#iyuuSiteSearch").oninput = (e) => renderSites(e.target.value);
      modal.querySelector("#iyuuAllSites").onclick = () => {
        sites.forEach((s) => selected.add(String(s.id || s.sid)));
        renderSites(modal.querySelector("#iyuuSiteSearch").value);
        this.toast(`已全选 ${selected.size} 个站点`);
      };
      modal.querySelector("#iyuuClearSites").onclick = () => {
        selected.clear();
        renderSites(modal.querySelector("#iyuuSiteSearch").value);
        this.toast("已清空站点选择");
      };
      modal.querySelector("#iyuuRefreshSites").onclick = async () => {
        const token = tokenInput.value.trim();
        if (!token) {
          this.toast("请先填写 IYUU Token");
          return;
        }
        try {
          const first = !Store.get(KEYS.configured, false);
          sites = await SiteIndex.get(true, token);
          if (first) {
            selected.clear();
            sites.forEach((s) => selected.add(String(s.id || s.sid)));
          }
          renderSites(modal.querySelector("#iyuuSiteSearch").value);
          this.toast(`站点索引已刷新：${sites.length} 个`);
        } catch (e) {
          this.toast(`刷新失败：${e.message}`);
        }
      };
      const saveConfig = /* @__PURE__ */ __name(() => Config.save({
      ...
      modal.querySelector("#iyuuDetectSites").onclick = async () => {
        try {
          MoviePilot.save(modal);
          const detected = SiteIndex.matchMoviePilot(await MoviePilot.sites(), sites);
          detected.forEach((sid) => selected.add(sid));
          renderSites(modal.querySelector("#iyuuSiteSearch").value);
          this.toast(`已按 MoviePilot 选择站点：${detected.length} 个`);
        } catch (e) {
          this.toast(`MoviePilot 获取失败：${e.message}`);
        }
      };
      modal.querySelector("#iyuuDetectLogin").onclick = async (ev) => {
        const b = ev.currentTarget;
        if (b.disabled) return;
        if (!confirm("该方式会逐站访问首页检测登录，较慢且较重，确定继续？")) return;
        b.disabled = true;
        const detected = await SiteIndex.detectLoggedIn(sites, (done, total) => {
          b.textContent = `检测中 ${done}/${total}`;
        });
        detected.forEach((sid) => selected.add(sid));
        renderSites(modal.querySelector("#iyuuSiteSearch").value);
        b.disabled = false;
        b.textContent = "选择已登录";
        this.toast(`已选择疑似登录站点：${detected.length} 个`);
      };
      modal.querySelector("#iyuuClearCache").onclick = () => ResultCache.clear();
      modal.querySelector("#iyuuResetConfig").onclick = () => Config.reset();
      modal.querySelector(".iyuu-cancel").onclick = () => bg.remove();
      bg.onclick = (e) => {
        if (e.target === bg) bg.remove();
      };
      modal.querySelector(".iyuu-save").onclick = () => {
        saveConfig();
        MoviePilot.save(modal);
        this.toast("配置已保存");
      };
  ...
  // src/scripts/iyuu-reseed-checker/routing.js
  var isCurrentHostInCachedIndex = /* @__PURE__ */ __name(() => {
    ...
    return matched;
  }, "isCurrentHostInCachedIndex");
  var isMTeamHost = /* @__PURE__ */ __name(() => /(^|\.)m-team\.(cc|io|vip)$/.test(location.hostname), "isMTeamHost");
  var isMTeamDetail = /* @__PURE__ */ __name(() => isMTeamHost() && location.pathname.startsWith("/detail/"), "isMTeamDetail");

  // src/common/pt-common.js
  var createPTCommon = /* @__PURE__ */ __name(({
    defaultLabel,
    productId,
    gridLabelClass,
    gridContentClass
    ...
    const NS = "pt-helper";
    const TORRENT_LINK_SELECTORS2 = Object.freeze([
      'a[href*="download.php"]',
      'a[href*="download"]',
      'a[href$=".torrent"]',
      'a[href*="/dl/"]',
    ...
    const DOM = {
      ...
      qsa(selector, root = document) {
        try {
          return typeof root?.querySelectorAll === "function" ? Array.from(root.querySelectorAll(selector)) : [];
        } catch (_) {
          return [];
        }
      },
      closest(node, selector) {
        try {
          return node ? Native.closest.call(node, selector) : null;
        } catch (_) {
          return null;
        }
      },
    ...
    const labelOf = /* @__PURE__ */ __name((mount) => mount?.label || defaultLabel, "labelOf");
    const rootSelector = /* @__PURE__ */ __name((siteId) => DOM.productRootSelector(siteId), "rootSelector");
    const stableNodeKey = /* @__PURE__ */ __name((node) => {
      if (!node) return "";
    ...
    const mountAnchorKey = /* @__PURE__ */ __name((mount) => {
    ...
    const mountHost = /* @__PURE__ */ __name((mount) => {
    ...
    const isEmptyElement = /* @__PURE__ */ __name((node) => node?.nodeType === 1 && !String(node.textContent || "").trim() && !node.querySelector?.("img,button,a,input,select,textarea,svg,canvas,video,audio"), "isEmptyElement");
    const cleanupEmptyMountShell = /* @__PURE__ */ __name((node) => {
      let current = node;
    ...
    }, "cleanupEmptyMountShell");
    const Mount2 = {
      afterNode(target) {
        return { type: "div-after", target };
      },
      inlineAfter(target, label = defaultLabel) {
        return { type: "inline-after", target, label };
      },
      tableRowAfter(target, label = defaultLabel) {
        return { type: "table-row-after", target, label };
      },
      tableColspanAfter(target, label = defaultLabel, colspan = 0) {
        return { type: "table-colspan-after", target, label, colspan };
      },
      tableColspanBefore(target, label = defaultLabel, colspan = 0) {
        return { type: "table-colspan-before", target, label, colspan };
      },
      blockAfter(target, label = defaultLabel) {
        return { type: "block-after", target, label };
      },
      antRowAfter(target, label = defaultLabel) {
        return { type: "ant-row-after", target, label };
      },
      gridPairAfter(target, label = defaultLabel) {
        return { type: "grid-pair-after", target, label };
      },
      prepend(target = document.body) {
        return { type: "prepend", target };
      },
      append(target) {
        return { type: "append", target };
      },
      definitionAfter(target, label = defaultLabel) {
        return { type: "definition-after", target, label };
      },
      ...
      tableRow(m, contentNode) {
        ...
        ["align", "valign"].forEach((attr) => {
          if (refHead?.getAttribute?.(attr)) h.setAttribute(attr, refHead.getAttribute(attr));
          if (refBody?.getAttribute?.(attr)) d.setAttribute(attr, refBody.getAttribute(attr));
        });
    ...
    const GazelleSites2 = {
      isOrpheusHost() {
        return /(^|\.)orpheus\.network$/i.test(location.hostname);
      },
      isGpwHost() {
        return location.hostname === "greatposterwall.com";
      },
      isHaidanHost() {
        return /(^|\.)haidan\.(cc|video)$/i.test(location.hostname);
      },
      orphusMatches() {
        return this.isOrpheusHost() && location.pathname === "/torrents.php" && new URLSearchParams(location.search).has("id");
      },
      gpwMatches() {
      ...
      haidanMatches() {
        return this.isHaidanHost() && location.pathname === "/details.php" && new URLSearchParams(location.search).has("group_id");
      },
      gazelleTidFromLink(link, param = "id") {
        const href = link?.href || link?.getAttribute?.("href") || "";
        try {
          return new URL(href, location.origin).searchParams.get(param) || "";
        } catch (_) {
          return "";
        }
      },
      ...
      gazelleUnifiedEntry(row, dl, opts = {}) {
        ...
        const format = String(opts.format || "").trim() || DOM.qs(".torrent_format, .format", row)?.textContent?.trim() || text.match(/\b(FLAC|MP3|AAC|AC3|DTS|Blu-?ray|WEB-?DL|WEBRip|Encode|Remux|DVD|HDTV|x264|x265|H\.?264|H\.?265)\b/i)?.[0] || "";
      ...
      gazelleHeader() {
        return DOM.qs("tr.colhead_dark") || DOM.qs("table.TableTorrent tr") || DOM.qsa("tr").find((tr) => /^\s*Torrents\s*$/i.test(tr.textContent || "")) || DOM.qs("tr");
      },
      orphusMount(label = defaultLabel) {
        ...
        if (tid) {
          ...
          return row ? Mount2.tableColspanAfter(row, label) : Mount2.afterNode(DOM.qs("h2") || document.body);
        }
        const header = this.gazelleHeader();
        return header ? Mount2.tableColspanAfter(header, label) : Mount2.afterNode(DOM.qs("h2") || document.body);
      },
      gpwMount(label = defaultLabel) {
        const tid = new URLSearchParams(location.search).get("torrentid");
        const header = DOM.qs("table.TableTorrent tr") || this.gazelleHeader();
        if (header) return Mount2.tableColspanAfter(header, label);
        if (tid) {
          const dl = DOM.qs(`a[href*="action=download"][href*="id=${tid}"]`);
          const row = this.gpwTorrentRow(tid) || DOM.closest(dl, "tr");
          return row ? Mount2.tableColspanAfter(row, label) : Mount2.afterNode(DOM.qs(`#torrent${tid}`) || document.body);
        }
        return Mount2.afterNode(document.body);
      },
      haidanMount(label = defaultLabel) {
        const header = DOM.qs(".torrents.content-color")?.firstElementChild;
        return Mount2.inlineAfter(header || DOM.qs(".detail-info-title") || DOM.qs(".detail-info-body") || document.body, label);
      },
      gpwTorrentRow(tid = new URLSearchParams(location.search).get("torrentid")) {
        if (!tid) return null;
        return DOM.qs(`#torrent${tid}, #torrent_${tid}, #torrent_detail_${tid}`) || DOM.closest(DOM.qs(`#torrent_details a[href*="id=${tid}"]`), "tr");
      },
...
${root?.textContent || ""}`.match(/Hash[：:]\s*([a-fA-F0-9]{40})/)?.[1] || "";
      ...
      async haidanResolveGroupId(tid, { origin = "", fetcher = null } = {}) {
        ...
        try {
          ...
          const html = typeof fetcher === "function" ? await fetcher(url) : await (await fetch(url, { credentials: "include" })).text();
    ...
    const SITE_FAMILIES2 = Object.freeze({
    ...
    function needsDownloadForHash2(info = {}) {
      const host = String(info.host || info.hostname || "").replace(/^www\./i, "").toLowerCase();
      const siteId = String(info.id || info.siteId || "").toLowerCase();
      const family = info.family || SITE_FAMILIES2[siteId] || SITE_FAMILIES2[host] || SITE_FAMILIES2[host.split(".")[0]] || "";
    ...
    __name(needsDownloadForHash2, "needsDownloadForHash");
    function tableMount2(siteId, row, label) {
      if (!row) return null;
      return Mount2.tableRowAfter(row, label);
    }
    __name(tableMount2, "tableMount");
    const AdapterRuntime2 = {
      withMount(findMount, getInfo) {
        return {
          findMount,
          getInfo: /* @__PURE__ */ __name((mount) => getInfo(mount || findMount()), "getInfo")
        };
      },
      siteId(adapter = {}) {
        return adapter.id || "";
      ...
      family(adapter = {}) {
        ...
        return adapter.family || SITE_FAMILIES2[siteId] || "";
      },
      mountRoot(adapter, mount, contentNode, anchorReason = "lazy-button") {
        const siteId = this.siteId(adapter);
        return Mount2.mountRoot(mount, contentNode, {
          siteId,
    ...
    const GazellePicker2 = {
      ...
      buildPanel({ title = "选择种子", rows = [], actionLabel = "选择", onAction = /* @__PURE__ */ __name(() => {
      }, "onAction") } = {}) {
        this.ensureStyle();
    ...
    const AutoFeedAnchors2 = {
      actionLabels: /* @__PURE__ */ new Set(["行为", "小货车", "行為", "种子认领", "簡介", "简介", "操作", "Action", "Tagline", "Tools:", "设备"]),
      nameLabels: /* @__PURE__ */ new Set(["Name", "Nombre", "名称", "标题"]),
      cellText(cell) {
      ...
      rowAfterName(root) {
        return this.rowByFirstCell(root, this.nameLabels);
      },
      bhdNameRow() {
        return this.rowAfterName(document.querySelector(".table-details tbody"));
      },
      monikaNameRow() {
      ...
      gpwTorrentRow() {
        const tid = new URLSearchParams(location.search).get("torrentid");
        if (!tid) return null;
        return document.querySelector(`#torrent${tid}, #torrent_${tid}, #torrent_detail_${tid}`) || document.querySelector(`#torrent_details a[href*="id=${tid}"]`)?.closest("tr");
      },
      hhclubSubtitleValue() {
        let label = null;
      ...
      hdSpaceMediaInfoRow() {
        const detailLabels = /* @__PURE__ */ new Set(["豆瓣 (NaN)", "评分", "类型", "国家/地区", "导演", "语言", "上映日期", "片长", "演员", "Year", "Runtime", "Country", "Genre", "Rating", "Votes", "Tagline", "Plot", "Cast"]);
    ...
    return { DOM, Mount: Mount2, SITE_FAMILIES: SITE_FAMILIES2, TORRENT_LINK_SELECTORS: TORRENT_LINK_SELECTORS2, needsDownloadForHash: needsDownloadForHash2, tableMount: tableMount2, AutoFeedAnchors: AutoFeedAnchors2, AdapterRuntime: AdapterRuntime2, GazelleSites: GazelleSites2, GazellePicker: GazellePicker2 };
  }, "createPTCommon");

  // src/scripts/iyuu-reseed-checker/page-tools.js
  var { DOM: PTDOM, Mount, SITE_FAMILIES, TORRENT_LINK_SELECTORS, needsDownloadForHash, tableMount, AutoFeedAnchors, AdapterRuntime, GazelleSites, GazellePicker } = createPTCommon({
    defaultLabel: "IYUU",
    productId: "iyuu",
  ...
  // src/scripts/iyuu-reseed-checker/crypto.js
  var Crypto = {
    bytes(input) {
    ...
    sha1Fallback(input) {
      ...
      data.set(bytes);
      data[len] = 128;
      const bitHi = Math.floor(len / 536870912);
      const bitLo = len << 3 >>> 0;
      data[size - 8] = bitHi >>> 24;
      data[size - 7] = bitHi >>> 16;
      data[size - 6] = bitHi >>> 8;
      data[size - 5] = bitHi;
      data[size - 4] = bitLo >>> 24;
      data[size - 3] = bitLo >>> 16;
      data[size - 2] = bitLo >>> 8;
      data[size - 1] = bitLo;
      let h0 = 1732584193, h1 = 4023233417, h2 = 2562383102, h3 = 271733878, h4 = 3285377520;
      const rotl = /* @__PURE__ */ __name((v, n) => v << n | v >>> 32 - n, "rotl");
    ...
    async sha1Hex(input) {
      ...
      return this.sha1Fallback(bytes);
    }
  };

  // src/scripts/iyuu-reseed-checker/torrent-fields.js
  var Helpers = {
    ...
    size(t) {
      const m = String(t || "").replace(/iB/gi, "B").toUpperCase().match(/(\d+(?:\.\d+)?)\s*(TB|GB|MB|KB)/);
      if (!m) return 0;
      return Number(m[1]) * ({ TB: 1024 ** 4, GB: 1024 ** 3, MB: 1024 ** 2, KB: 1024 }[m[2]] || 1);
    },
    findDownload(selectors) {
      for (const sel of selectors) {
        const el = document.querySelector(sel);
        if (el?.href || el?.value) return el.href || el.value;
      }
      return "";
    },
    scoreTorrentLink(link, { allowMagnet = false, rewardDownloadPath = false } = {}) {
      const h = String(link?.href || link || "");
      if (!h || h.startsWith("magnet:") && !allowMagnet) return -1;
      let s = 0;
      if (/^https:/i.test(h)) s += 2;
      if (/passkey=/i.test(h)) s += 2;
      if (/https=1/i.test(h)) s += 1;
      if (rewardDownloadPath && /download\.php|\/download(?:\/|$)|\.torrent(?:$|\?)/i.test(h)) s += 3;
      if (/edit|report|comment|help|rules/i.test(h)) s -= 2;
      return s;
    },
    titleFromDownload(link) {
      return String(link?.textContent || link?.href?.split("/").pop() || "").replace(/\.torrent$/i, "").trim();
    },
    info({ id, name, description = "", downloadLink = "", sizeText = "", size = 0, mount, extra = {} }) {
      return name && mount?.target ? { id, name: String(name).trim(), description, downloadLink, size: Number(size) || this.size(sizeText), mount, extra } : null;
    }
  };

  // src/scripts/iyuu-reseed-checker/infohash.js
  var InfoHash = {
    async extract(info) {
      ...
      if (magnetHash) {
        log("hash from magnet", magnetHash);
        return magnetHash;
      }
      const specialHash = await this.fromSpecial(info);
      if (specialHash) {
        log("hash from special site/torrent", specialHash);
        return specialHash;
      }
      ...
      const torrentHash = await this.fromTorrent(torrentUrl).catch((e) => {
        log("torrent hash failed", e.message);
        return "";
      });
      if (torrentHash) {
        log("hash from torrent", torrentHash);
        return torrentHash;
      }
    ...
    fromMagnet(v) {
      const m = String(v || "").match(/xt=urn:btih:([a-z2-7]{32}|[a-f0-9]{40})/i);
      if (!m) return "";
      return m[1].length === 40 ? m[1].toLowerCase() : this.base32ToHex(m[1]);
    },
    fromPage() {
      const text = `${location.href}
${document.body?.innerText || ""}`;
      const all = text.match(/\b[a-fA-F0-9]{40}\b/g) || [];
      return firstOf(all, this.valid) || "";
    },
    valid(h) {
      return /^[a-f0-9]{40}$/i.test(h) && !/^(.)\1{39}$/.test(h) && !/^\d{40}$/.test(h);
    },
    base32ToHex(s) {
      const alpha = "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567";
      let bits = "";
      String(s).toUpperCase().replace(/=+$/, "").split("").forEach((c) => {
        const v = alpha.indexOf(c);
        if (v >= 0) bits += v.toString(2).padStart(5, "0");
      });
      let out = "";
      for (let i = 0; i + 4 < bits.length; i += 4) out += parseInt(bits.slice(i, i + 4), 2).toString(16);
      return out.slice(0, 40).toLowerCase();
    },
    async fromSpecial(info) {
      if (info._bangumiId) {
        const r = await HTTP.request({ url: `https://bangumi.moe/api/v2/torrent/${info._bangumiId}` });
        if (r?.magnet) {
          info.downloadLink = r.magnet;
          return this.fromMagnet(r.magnet);
        }
      }
      if (/m-team\.(cc|io|vip)/.test(location.hostname)) {
        const link = await this.mteamLink();
        if (link) {
          info.downloadLink = link;
          return await this.fromTorrent(link);
        }
      }
      return "";
    },
    async mteamLink() {
      const id = location.pathname.match(/\/detail\/(\d+)/)?.[1];
      if (!id || !Config.mteamKey) return "";
      const res = await HTTP.request({ method: "POST", url: `${MTEAM_API_BASE}/torrent/genDlToken`, data: `id=${encodeURIComponent(id)}`, headers: { "Content-Type": "application/x-www-form-urlencoded", "x-api-key": Config.mteamKey }, responseType: "json" });
      return res?.data || "";
    },
    async fromTorrent(url) {
      ...
      try {
        ...
        throw new Error(`下载种子失败：${e?.message || e}`);
      }
    ...
    assertTorrentResponse(bytes) {
      ...
      if (/^\s*\{/.test(head)) {
        try {
          ...
          throw new Error(json?.message || json?.msg || "下载链接未返回种子文件");
        } catch (e) {
          if (e?.message) throw e;
          throw new Error("下载链接未返回种子文件");
        }
      }
    ...
    infoRange(bytes) {
      ...
      const key = str.indexOf("4:info");
      if (key < 0) return null;
      const start = key + 6;
      let i = start;
      const walk = /* @__PURE__ */ __name(() => {
        const begin = i;
        const c = str[i];
        if (c === "i") {
          i = str.indexOf("e", i) + 1;
        ...
        if (c === "l" || c === "d") {
          i++;
          while (str[i] !== "e" && i < str.length) walk();
          i++;
          return [begin, i];
        }
        if (/\d/.test(c)) {
  ...
  // src/scripts/iyuu-reseed-checker/reseed-sites.js
  var ReseedSite = {
    sid(raw = {}, fallback = "") {
    ...
    name(raw = {}, meta = {}, sid = "", source = "iyuu") {
      return raw.site_name || raw.site_alias || raw.name || raw.site || raw.nickname || raw.name_cn || SiteIndex.name(meta) || (source === "iyuu" ? `站点${sid}` : "IYUU");
    },
  ...
  // src/scripts/iyuu-reseed-checker/iyuu-api.js
  var IYUU = {
      ...
      for (let attempt = 1; attempt <= retries; attempt++) {
        try {
          ...
          if (res?.code === 400 && String(res?.msg || "").includes("Server internal error") && attempt < retries) {
            log(`reportExisting Server internal error, waiting ${attempt * 1e3}ms before retry...`);
          ...
          if (attempt < retries) {
            log(`reportExisting error, waiting ${attempt * 1e3}ms before retry...`, e);
            await new Promise((r) => setTimeout(r, attempt * 1e3));
            continue;
          }
          throw e;
    ...
    async query(hash, retried = false, attempt = 1) {
      ...
      const hashes = Array.from(/* @__PURE__ */ new Set([hash.toLowerCase()])).sort();
      const sha1 = await Crypto.sha1Hex(JSON.stringify(hashes));
      const params = new URLSearchParams({ hash: JSON.stringify(hashes), sha1, sid_sha1, timestamp: String(Math.floor(Date.now() / 1e3)), version: "8.2.0" });
      ...
      const retryableHttpError = raw && raw.code !== void 0 && raw.code !== 0 && this.retryableQueryError(`${raw.code} ${raw.msg || ""}`);
      ...
      if (raw && raw.code !== void 0 && raw.code !== 0) throw new Error(raw.msg || "IYUU 查询失败");
  ...
  // src/scripts/iyuu-reseed-checker/fallback.js
  var Fallback = {
    items(raw) {
    ...
    isOwnedItem(item, built, ownedSet, ownedSites) {
      ...
      const hostCandidates = [
        built?.host,
        SiteIndex.host(built),
        SiteIndex.host({ url: built?.downloadUrl }),
        item?.domain,
        item?.host,
        item?.base_url,
        item?.url,
  ...
  // src/scripts/iyuu-reseed-checker/adapters.js
  var ADAPTERS = [
    {
      id: "totheglory",
      matches: /* @__PURE__ */ __name(() => location.hostname === "totheglory.im" && location.pathname.startsWith("/t/"), "matches"),
    ...
    {
      id: "hdsky",
      matches: /* @__PURE__ */ __name(() => location.hostname === "hdsky.me" && location.pathname === "/details.php", "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "sjtu",
      matches: /* @__PURE__ */ __name(() => location.hostname === "pt.sjtu.edu.cn" && location.pathname === "/details.php", "matches"),
      ...AdapterRuntime.withMount(
        () => {
          const rows = [...document.querySelectorAll(".rowhead,.heading")];
    ...
    {
      id: "bangumi",
      matches: /* @__PURE__ */ __name(() => location.hostname === "bangumi.moe", "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "mikanani",
      matches: /* @__PURE__ */ __name(() => location.hostname === "mikanani.me" && location.pathname.includes("/Home/Episode/"), "matches"),
      ...AdapterRuntime.withMount(
    ...
    {
      id: "m-team",
      matches: /* @__PURE__ */ __name(() => /m-team\.(cc|io|vip)\/detail\//.test(location.href), "matches"),
      findMount: /* @__PURE__ */ __name(() => {
    ...
    {
      id: "hdcity",
      matches: /* @__PURE__ */ __name(() => location.hostname === "hdcity.city" && /^\/t-/.test(location.pathname), "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "monikadesign",
      matches: /* @__PURE__ */ __name(() => location.hostname === "monikadesign.uk" && /^\/torrents\/\d+\/?$/.test(location.pathname), "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "beyond-hd",
      matches: /* @__PURE__ */ __name(() => location.hostname === "beyond-hd.me" && location.pathname.startsWith("/torrents/"), "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "eiga",
      matches: /* @__PURE__ */ __name(() => location.hostname === "eiga.moi" && location.pathname.startsWith("/torrents/"), "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "hd-space",
      matches: /* @__PURE__ */ __name(() => location.hostname === "hd-space.org" && location.search.includes("page=torrent-details"), "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "iptorrents",
      matches: /* @__PURE__ */ __name(() => location.hostname === "iptorrents.com" && location.pathname === "/torrent.php", "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "filelist",
      matches: /* @__PURE__ */ __name(() => location.hostname === "filelist.io" && location.pathname === "/details.php", "matches"),
      ...AdapterRuntime.withMount(
        () => {
    ...
    {
      id: "hudbt",
      matches: /* @__PURE__ */ __name(() => location.hostname === "hudbt.hust.edu.cn" && location.pathname === "/details.php", "matches"),
      ...AdapterRuntime.withMount(
        () => {
          const dl = document.querySelector('a.index[href*="download.php?id="]') || document.querySelector('a[href*="download.php?id="]');
          const table = document.querySelector("#outer dl.table");
          const dts = [...document.querySelectorAll("#outer dl.table > dt")];
          const byLabel = /* @__PURE__ */ __name((label) => firstOf(dts, (dt) => dt.textContent.includes(label))?.nextElementSibling, "byLabel");
    ...
    {
      id: "greatposterwall",
      matches: /* @__PURE__ */ __name(() => GazelleSites.gpwMatches(), "matches"),
      ...AdapterRuntime.withMount(
    ...
    {
      id: "haidan",
      matches: /* @__PURE__ */ __name(() => GazelleSites.haidanMatches(), "matches"),
      ...AdapterRuntime.withMount(
    ...
    {
      id: "hhclub",
      matches: /* @__PURE__ */ __name(() => location.hostname === "hhanclub.net" && location.pathname === "/details.php", "matches"),
      ...AdapterRuntime.withMount(
        () => {
          const dl = document.querySelector('a.index[href*="download.php?id="]') || document.querySelector('a[href*="download.php?id="]');
          const row = dl?.closest("tr");
    ...
    {
      id: "nyaa",
      matches: /* @__PURE__ */ __name(() => location.hostname === "nyaa.si", "matches"),
      ...AdapterRuntime.withMount(
    ...
    {
      id: "acg-rip",
      matches: /* @__PURE__ */ __name(() => location.hostname === "acg.rip", "matches"),
      ...AdapterRuntime.withMount(
    ...
    {
      id: "comicat-kisssub",
      matches: /* @__PURE__ */ __name(() => /(^|\.)(comicat|kisssub)\.org$/i.test(location.hostname), "matches"),
      ...AdapterRuntime.withMount(
    ...
    {
      id: "generic",
      matches: /* @__PURE__ */ __name(() => true, "matches"),
      ...AdapterRuntime.withMount(
        () => {
          const rows = [...document.querySelectorAll(".rowhead,.heading")];
          const nameRow = rows[0];
  ...
  // src/scripts/iyuu-reseed-checker/core.js
  var Core = {
    adapter: null,
    async init() {
    ...
    async checkOrPick(box, btn, info) {
      ...
      const liveInfo = liveEntries.length > 1 ? { ...info, extra: { ...info.extra || {}, groupMode: true, entries: liveEntries, groupTitle: info.extra?.groupTitle || info.name, currentTid: info.extra?.tid || "" } } : info;
    ...
    inject(info) {
      ...
      box.className = "iyuu-row-box pt-helper-root pt-helper-root-iyuu";
      const btn = document.createElement("button");
      btn.className = "iyuu-btn iyuu-check-btn";
      this.setQueryButton(btn, "未查询", "配置请从 Tampermonkey 菜单打开「配置 IYUU」");
      btn.onclick = async (e) => {
        ...
        e.stopPropagation();
        try {
          await this.checkOrPick(box, btn, box._iyuuLastInfo?.extra?.groupMode === false ? box._iyuuLastInfo : info);
        } catch (err) {
      ...
      multiInput.onchange = (e) => {
        e.stopPropagation();
        box.querySelectorAll(".iyuu-result").forEach((n) => n.remove());
        if (box._iyuuResult) this.render(box, btn, box._iyuuResult, info, box._iyuuCached);
      };
      box.append(btn, multi);
      AdapterRuntime.mountRoot({ id: info.id || "" }, info.mount, box, info.mount?.type || "");
    ...
    showPicker(box, btn, info) {
      ...
      const panel = GazellePicker.buildPanel({
        ...
        rows: entries,
        actionLabel: "选择并查询",
        onAction: /* @__PURE__ */ __name(async (entry, actionBtn) => {
    ...
    download(url) {
      ...
      if (url.startsWith("magnet:")) {
        UI.toast("磁力链接，已在新标签打开");
        this.openTab(url);
        return;
      }
      if (SiteIndex.isLikelyDetailUrl(url)) {
        UI.toast("该站点缺少下载链接，已打开详情页");
        this.openTab(url);
        return;
      }
      try {
        ...
        a.href = url;
        a.download = (url.split("/").pop() || "download").split("?")[0] || "download.torrent";
        a.style.display = "none";
        document.body.appendChild(a);
        a.click();
        a.remove();
      } catch (e) {
        UI.toast(`下载失败：${e.message}，改为打开链接`);
        this.openTab(url);
      }
    },
    ...
    async downloadSelected(selected) {
      ...
      try {
        ...
        for (const site of sites) {
          const url = await this.resolveDownloadUrl(site);
          if (url) urls.push(url);
          else missing.push(SiteIndex.name(site));
        }
        if (!urls.length) {
          UI.toast(missing.length ? `${missing.join("、")} 缺少下载链接` : "选中站点缺少下载链接");
          return;
        }
        if (missing.length) UI.toast(`${missing.join("、")} 缺少下载链接，已跳过`);
        if (urls.length === 1) {
          this.download(urls[0]);
          UI.toast("已触发下载 1 个站点种子");
          return;
        }
        urls.forEach((url, i) => setTimeout(() => this.openTab(url), i * 500));
        UI.toast(`已分批打开 ${urls.length} 个站点下载链接`);
      } catch (e) {
    ...
    siteKeys(s) {
      ...
      const add = /* @__PURE__ */ __name((type, value) => {
        const v = String(value || "").trim().toLowerCase();
        if (v) keys.add(`${type}:${v}`);
      }, "add");
    ...
    mergeSites(list) {
      ...
      (list || []).forEach((s) => {
        ...
        const index = firstOf(keys.map((k) => keyToIndex.get(k)), (i) => i !== void 0);
        if (index === void 0) {
    ...
    async injectCurrentSite(result, info) {
      ...
      const isCurrent = /* @__PURE__ */ __name((s) => sid && sid === String(s?.sid || "") || s?.host && SiteIndex.sameHost(currentHost, s.host), "isCurrent");
      let matched = false;
      result.sites = result.sites.map((s) => {
        if (s?.source === "current" && !isCurrent(s)) return { ...s, source: s._origSource || "iyuu" };
        if (isCurrent(s)) {
          matched = true;
          return { ...s, _origSource: s._origSource || (s.source === "current" ? "iyuu" : s.source), source: "current" };
        }
        return s;
      });
      if (!matched) {
    ...
    sourceErrors(result) {
      const parts = (result.sources || []).filter((s) => s?.ok === false && s?.error || s?.source === "fallback" && s?.ok === true && !(s?.sites || []).length).map((s) => s?.source === "fallback" && s?.ok === true ? "无结果（ZMPT）" : `${s.error}（${s.source === "iyuu" ? "IYUU" : s.source === "fallback" ? "ZMPT" : s.source}）`);
      return parts.length ? [parts.join(" | ")] : [];
    },
    async fetch(hash) {
      log("fetch sources", { hash, iyuu: Boolean(Config.token), fallback: Config.zmpt });
      const sources = [];
      const iyuuResult = Config.token ? await IYUU.query(hash).catch((e) => ({ ok: false, source: "iyuu", error: e?.message || "IYUU 查询失败", sites: [] })) : { ok: false, source: "iyuu", error: "未配置 IYUU Token", sites: [] };
    ...
    async enrichHaidanGroupId(sites) {
      ...
      await Promise.all(targets.map(async (s) => {
        ...
        const gid = await GazelleSites.haidanResolveGroupId(tid, {
          ...
          fetcher: /* @__PURE__ */ __name((url) => HTTP.request({ url, responseType: "text" }).catch(() => ""), "fetcher")
        });
    ...
    render(box, btn, result, info, cached = false) {
      ...
      box._iyuuResult = result;
      box._iyuuCached = cached;
      box.querySelectorAll(".iyuu-result").forEach((n) => n.remove());
      const wrap = document.createElement("span");
      wrap.className = "iyuu-result iyuu-row-box";
      const visible = this.visibleSites(result.sites);
      const selected = box._iyuuSelected || new Set(result.sites);
      const multi = Boolean(box.querySelector(".iyuu-multi-toggle")?.checked);
      const primary = firstOf(result.sources, (s) => s.source === "iyuu");
      const hasSites = Boolean(visible.length);
      const primaryError = primary?.ok === false && primary.error ? primary.error : "";
      const srcErrors = this.sourceErrors(result);
      ...
      if (info.extra?.groupMode === false && (info.extra?.entries || []).length > 1) {
        ...
        reselect.onclick = (e) => {
          e.preventDefault();
          e.stopPropagation();
          this.showPicker(box, btn, { ...info, extra: { ...info.extra || {}, groupMode: true } });
        };
    ...
    renderMultiActions(wrap, box, btn, result, info, cached, selected, visible) {
      ...
      all.className = "iyuu-btn";
      all.textContent = "全选";
      all.onclick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        visible.forEach((s) => selected.add(s));
        box.querySelectorAll(".iyuu-result").forEach((n) => n.remove());
        this.render(box, btn, result, info, cached);
      };
      const none = document.createElement("button");
      none.className = "iyuu-btn";
      none.textContent = "清空";
      none.onclick = (e) => {
        e.preventDefault();
        e.stopPropagation();
      ...
      dl.className = "iyuu-btn";
      dl.textContent = "下载种子";
      dl.onclick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        this.downloadSelected(selected);
      };
      const open = document.createElement("button");
      open.className = "iyuu-btn";
      open.textContent = "打开选中";
      open.onclick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        const urls = [...selected].filter((s) => s.url);
        if (!urls.length) {
          UI.toast("未勾选可跳转站点");
          return;
        }
        urls.forEach((s) => this.openTab(s.url));
        UI.toast(`已打开 ${urls.length} 个站点`);
      };
      wrap.append(all, none, dl, open);
    }
  };

  // src/scripts/iyuu-reseed-checker/startup.js
  function main() {
    Config.load();
    UI.initStyle();
  ...
  // src/scripts/iyuu-reseed-checker/index.js
  bootstrap();
})();
