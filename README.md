...
| <img src="icon/pt-batch.png" width="24" alt=""> [PT_BatchDownload](#pt_batchdownload) | 0.6.12 | 通用 PT 当前页批量下载 | [安装](https://cdn.jsdelivr.net/gh/wuyaos/greasyfork_scripts@main/dist/PT_BatchDownload.user.js) |
...
- ZIP 打包使用内置同步打包器（STORE 存储），不再依赖 JSZip 流管线，避免油猴沙箱异步调度失效导致打包卡死
- 打包失败时自动逐个下载已获取文件，不重复请求站点（浏览器可能需要允许多文件下载）
